...
*/	
        ...
        if (server && state && !state.ack) {
            ...
            if (clients[channelId]) {
                ...
                if (specVars.indexOf(stateId) === -1) {
                    ...
                    adapter.getForeignState(idAlive, (err, xState) => {
                        let ledFlags = 1;
                        if (!xState) {
    ...
    function addObject(attr, client, prefix, path) {
        ...
        let id = adapter.namespace + '.' + client.id + '.' + (prefix ? prefix + '.' : '') + (path.length ? path.join('_') + '_' : '') + replaceAttr.replace(FORBIDDEN_CHARS, '_');
        let obj = {
            type: 'addObject',
            id: id,
            data: {
        ...
        for (let attr in data) {
            ...
            if (typeof data[attr] === 'object') {
                ...
                if (attr === 'Color') {
                    ...
                    if (hueCalc) {
                        // This operation must be executed earlier
                        let idAlive = adapter.namespace + '.' + client.id + '.LED_Mode';
                        let xObj = addObject('LED_Mode', client, prefix, path);
                        xObj.data.common.def = 1;
                        updateState(xObj);
                        ...
                        updateState(xObj, ledFlags & 2 ? 100 * parseInt(data[attr].substring(0, 2), 16) / 255 : undefined);

                        xObj = addObject('Green', client, prefix, path);
                        updateState(xObj, ledFlags & 2 ? 100 * parseInt(data[attr].substring(2, 4), 16) / 255 : undefined);

                        xObj = addObject('Blue', client, prefix, path);
                        updateState(xObj, ledFlags & 2 ? 100 * parseInt(data[attr].substring(4, 6), 16) / 255 : undefined);

                        xObj = addObject('RGB_POWER', client, prefix, path);
                        let val = parseInt(data[attr].substring(0, 6), 16);
                        updateState(xObj, (ledFlags & 2) ? (val > 0) : undefined);

                        if (obj.data.common.role === 'level.color.rgbww') {
                            // rgbww
                            xObj = addObject('WW', client, prefix, path);
                            updateState(xObj, (ledFlags & 2) ? 100 * parseInt(data[attr].substring(6, 8), 16) / 255 : undefined);
                        ...
                        if (obj.data.common.role === 'level.color.rgbcwww') {
                            ...
                            xObj = addObject('WW', client, prefix, path);
                            updateState(xObj, (ledFlags & 2) ? 100 * parseInt(data[attr].substring(8, 10), 16) / 255 : undefined);
