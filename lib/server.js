...
function MQTTServer(adapter) {
    ...
    adapter.getForeignObjects(`${adapter.namespace}.*`, (err, objects) => {
        if (objects) {
            Object.assign(knownObjects, objects);
        }
    });
    adapter.getForeignStates(`${adapter.namespace}.*`, (err, states) => {
        if (states) {
            Object.assign(knownStates, states);
        }
    });
...
*/
                ...
                if (previousState && state.val === previousState.val) {
                    ...
                    if (ignoreAck || !isAck) {
                        ...
                        if (now - last < adapter.config.debounceTime) {
                            adapter.log.debug(
                                `Debounce ignored ${id} (${now - last}ms < ${adapter.config.debounceTime}ms)`,
                            );
                            return;
                        }
                    }
    ...
    function addObject(typeKey, client, prefix, path) {
        ...
        let attr = typeKey;
        if (path.length > 0) {
            const pathPrefix = `${path.join('_')}_`;
            if (typeKey.startsWith(pathPrefix)) {
