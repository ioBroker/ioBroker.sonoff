...
*/
    ...
    function receivedTopic(packet, client) {
        ...
        */
        ...
        // Check for numbered tags
        let stateId_n;
        if (stateId.match(/^A\d$/)          || // A1-9
            stateId.match(/^Analog\d$/)     || // Analog1-9
            stateId.match(/^C\d$/)          || // C1-9
            stateId.match(/^Counter\d$/)    || // Counter1-9
            stateId.match(/^POWER\d{0,2}$/) || // 99 POWER
            stateId.match(/^SHUTTER\d$/)    || // SHUTTER1-9
            stateId.match(/^Switch\d$/)     || // Switch1-9
            stateId.match(/^Timer\d$/))        // Timer1-9
        {    
            stateId_n = stateId.replace(/\d{0,2}$/, '_n');
        } else {
            stateId_n = stateId;
        }
        ...
        if (parts[0] === 'tele' && stateId.match(/^(STATE|SENSOR|WAKEUP)\d?$/)) {
        ...
        } else if (types[stateId] || types[stateId_n]) {
            ...
            let obj = {
                ...
                data: {
                    _id: id,
                    common: JSON.parse(JSON.stringify(types[stateId_n])),
                    native: {},
                    type: 'state'
                }
            };
