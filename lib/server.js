...
function MQTTServer(adapter) {
    ...
    const cachePowerObjects = {};
    const knownObjects = {};
    const knownStates = {};
    const debounceTimes = {};

    // Initialize caches
    adapter.getForeignObjects(`${adapter.namespace}.*`, (err, objects) => {
        if (objects) { Object.assign(knownObjects, objects); }
    });
    adapter.getForeignStates(`${adapter.namespace}.*`, (err, states) => {
        if (states) { Object.assign(knownStates, states); }
    });
...
*/
    ...
    function setColor(channelId, val) {
        ...
        if (clients[channelId] && clients[channelId]._map && clients[channelId]._map[stateId]) {
            setImmediate(
                sendState2Client,
                clients[channelId],
                clients[channelId]._map[stateId] || 'cmnd/sonoff/Color',
                val,
    ...
    function setPower(channelId, val) {
        ...
        if (clients[channelId]._map && clients[channelId]._map[stateId]) {
            setImmediate(
                sendState2Client,
                clients[channelId],
                clients[channelId]._map[stateId] || 'cmnd/sonoff/POWER',
                val ? 'ON' : 'OFF',
                adapter.config.defaultQoS,
            );
    ...
    function setZbSendCommand(channelId, deviceId, attribute, val) {
        ...
        if (clients[channelId] && clients[channelId]._fallBackName) {
            setImmediate(
                sendState2Client,
                clients[channelId],
                `cmnd/${clients[channelId]._fallBackName}/ZbSend`,
                zbSendCommand,
                adapter.config.defaultQoS,
            );
        } else {
            adapter.log.warn(`Cannot send ZbSend command for device ${deviceId}: client not found`);
        }
    }
    ...
    function _setState(id, val, callback) {
        ...
        adapter.setForeignState(id, val, true, () => {
            if (!knownStates[id]) knownStates[id] = {};
            knownStates[id].val = val;
            knownStates[id].ack = true;
            if (callback) { callback() };
        });
    }
    ...
    function updateState(task, val, callback) {
        ...
        adapter.log.debug(`Update state ${task.id} - ${tasks.length}`);
        if (tasks.length === 1) {
            processTasks();
        }
    }
    ...
    function onStateChangedColors(id, state, channelId, stateId) {
        ...
        if (stateId.match(/Color\d?/)) {
            ...
            if (stateId.match(/Red\d?/)) {
            ...
            } else if (stateId.match(/RGB_POWER\d?/)) {
                // set ww component
            ...
            } else if (hueCalc && stateId.match(/Hue\d?/)) {
            ...
            } else if (hueCalc && stateId.match(/Saturation\d?/)) {
                let sat = state.val;
                if (sat > 100) {
                    sat = 100;
                }
                ...
                adapter.getForeignObject(idAlive, (err, obj) => {
                    if (!obj) {
                        ...
                        } else if (stateId.match(/CW_POWER\d?/)) {
                            // set ww component
                            const cwpow =
                                state.val === 'true' || state.val === true || state.val === 1 || state.val === '1';
                            const idAlive = `${deviceDesc}.Color`;
    ...
    this.onStateChange = (id, state) => {
        ...
        const previousState = knownStates[id];
        if (state) {
            knownStates[id] = state;
        } else {
            delete knownStates[id];
        }
        if (server && state && !state.ack) {
            // Debounce logic
            if (adapter.config.debounceTime > 0) {
                if (previousState && state.val === previousState.val) {
                    const ignoreAck = adapter.config.debounceIgnoreAck;
                    const isAck = previousState.ack;

                    if (ignoreAck || !isAck) {
                        const now = Date.now();
                        const last = debounceTimes[id] || 0;
                        if (now - last < adapter.config.debounceTime) {
                            adapter.log.debug(`Debounce ignored ${id} (${now - last}ms < ${adapter.config.debounceTime}ms)`);
                            return;
                        }
                    }
                }
            }
            debounceTimes[id] = Date.now();
            ...
            if (zbMatch && clients[mappingClients[channelId]]) {
                ...
                adapter.log.debug(`Sending ZbSend command for device ${deviceId}: ${attribute}=${zbValue}`);
                setZbSendCommand(mappingClients[channelId], deviceId, attribute, zbValue);
                return;
            }
            ...
            if (clients[mappingClients[channelId]]) {
                ...
                if (!specVars.includes(stateId)) {
                    ...
                    adapter.getObject(id, (err, obj) => {
                        if (!obj) {
                            adapter.log.warn(`Invalid object ${id}`);
                        } else {
                ...
                if (!adapter.config.ignoreNotConnectedWarnings) {
                    adapter.log.info(`Client "${channelId}" not connected`);
                }
    ...
    function processTasks(callback, depth) {
        depth = depth || 0;
        if (callback) {
            taskCallbacks.push(callback);
        }
        ...
        const next = () => {
            tasks.shift();
            if (depth < 50) {
                processTasks(null, depth + 1);
            } else {
                setImmediate(() => processTasks(null, 0));
            }
        };
        ...
        if (task.type === 'addObject') {
            if (!cacheAddedObjects[task.id]) {
                ...
                const checkObj = obj => {
                    if (!obj || !obj.common) {
                        if (task.data.common.storeMap) {
                            task.data = JSON.parse(JSON.stringify(task.data)); // copy
                            delete task.data.common.storeMap;
                        }
                        adapter.setForeignObject(task.id, task.data, err => {
                            if (err) {
                                adapter.log.warn(`New object creation error: ${err.message}`);
                            } else {
                                knownObjects[task.id] = task.data;
                                adapter.log.info(`New object created: ${task.id}`);
                            }
                            if (task.cb) {
                                task.cb(task.id, task.cbArg, next);
                            } else {
                                next();
                            }
                        });
                    } else if (obj.common.type !== task.data.common.type || obj.common.storeMap !== undefined) {
                        obj.common.type = task.data.common.type;
                        if (obj.common.storeMap !== undefined) {
                            delete obj.common.storeMap;
                        }
                        adapter.setForeignObject(task.id, obj, err => {
                            if (err) {
                                adapter.log.warn(`Object update error: ${err.message}`);
                            } else {
                                knownObjects[task.id] = obj;
                ...
                if (knownObjects[task.id]) {
                    checkObj(knownObjects[task.id]);
                } else {
                    adapter.getForeignObject(task.id, (err, obj) => {
                        if (obj) { knownObjects[task.id] = obj; }
                        checkObj(obj);
                    });
                }
            } else {
            ...
            adapter.deleteObject('', '', task.id, (/* err */) => {
                if (task.cb) {
                    task.cb(task.id, task.cbArg, next);
                } else {
                    next();
    ...
    async function updateClients() {
        ...
        if (knownStates[id] && knownStates[id].val === val && knownStates[id].ack && !id.includes('.RESULT.')) {
            if (callback) { callback(); }
            return;
        }
                }
    ...
    function resendMessages2Client(client, messages, i) {
        ...
        if (messages && i < messages.length) {
            try {
                ...
                messages[i].count++;
                adapter.log.debug(
                    `Client [${client.id}] resend messages on connect: ${messages[i].topic} = ${messages[i].payload}`,
                );
    ...
    function syncPowerState(powerId, val) {
        const doSync = () => {
            adapter.getForeignState(powerId, (err, state) => {
                if (!state || state.val != val) {
                    adapter.log.debug(`Sync .STATE.POWER to .POWER: ${powerId}`);
                    adapter.setForeignState(powerId, val, true);
                }
            });
        };

        if (cachePowerObjects[powerId]) {
            doSync();
        } else {
            adapter.getForeignObject(powerId, (err, exists) => {
                if (exists) {
                    cachePowerObjects[powerId] = true;
                    doSync();
                }
            });
        }
            ...
            if (typeof data[attr] === 'object') {
                ...
                if (types[attr]) {
                    if (types[attr].type === 'array') {
                        ...
                        checkData(client, topic, prefix, data[attr], nestedUnits, nPath);
                        nPath = undefined;
                    }
                } else {
                    let nPath = Object.assign([], path);
                    ...
                    if (adapter.config.OBJ_TREE) {
                        checkData(client, topic, `${prefix}.${nPath}`, data[attr], nestedUnits);
                    } else {
                        checkData(client, topic, prefix, data[attr], nestedUnits, nPath);
                    }
                    nPath = undefined;
                ...
                if (
                    ...
                    let controlObj = Object.assign({}, obj);
                    ...
                    if (attr === 'Power') {
                        ...
                        updateState(controlObj, controlVal);
                    } else if (attr === 'Dimmer') {
                        ...
                        updateState(controlObj, val);
                    }

                    adapter.log.debug(`Created Zigbee control state for device ${deviceId}: ${controlObj.id}`);
                }

                // Sync .STATE.POWER -> .POWER
                if (obj.id.includes('.STATE.POWER')) {
                    const powerId = obj.id.replace('.STATE.POWER', '.POWER');
                    syncPowerState(powerId, val);
                }
            } else {
    ...
    function receivedTopic(packet, client) {
        ...
        adapter.log.debug(`Client [${client.id}] received: ${packet.topic} = ${val}`);
    ...
    function checkResends() {
        ...
        for (const clientId in clients) {
            if (
                Object.prototype.hasOwnProperty.call(clients, clientId) &&
                clients[clientId] &&
                clients[clientId]._messages
            ) {
                for (let m = clients[clientId]._messages.length - 1; m >= 0; m--) {
                    ...
                    if (now - message.ts >= adapter.config.retransmitInterval) {
                        ...
                        try {
                            adapter.log.debug(
                                `Client [${clientId}] resend message topic: ${message.topic}, payload: ${message.payload}`,
                            );
        ...
        if (adapter.config.storeClientsTime !== -1) {
            for (const id in persistentSessions) {
                if (Object.prototype.hasOwnProperty.call(persistentSessions, id)) {
    ...
    (function _constructor(config) {
        ...
        config.debounceIgnoreAck = config.debounceIgnoreAck === true || config.debounceIgnoreAck === 'true';
        ...
        server.on('connection', stream => {
            ...
            client.on('connect', options => {
                ...
                if (config.user) {
                    ...
                    if (config.user !== options.username || config.password !== options.password) {
            ...
            client.on('publish', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    !adapter.config.ignorePings &&
                        adapter.log.warn(
                            `Old client ${client.id} with secret ${client.__secret} sends publish. Ignore! Actual secret is ${clients[client.id].__secret}`,
                        );
                    return;
                }
            ...
            client.on('pubrec', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    !adapter.config.ignorePings &&
                        adapter.log.warn(
                            `Old client ${client.id} with secret ${client.__secret} sends pubrec. Ignore! Actual secret is ${clients[client.id].__secret}`,
                        );
                    return;
                }
            ...
            client.on('pubcomp', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    !adapter.config.ignorePings &&
                        adapter.log.warn(
                            `Old client ${client.id} with secret ${client.__secret} sends pubcomp. Ignore! Actual secret is ${clients[client.id].__secret}`,
                        );
                    return;
                }
            ...
            client.on('pubrel', packet => {
                ...
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    !adapter.config.ignorePings &&
                        adapter.log.warn(
                            `Old client ${client.id} with secret ${client.__secret} sends pubrel. Ignore! Actual secret is ${clients[client.id].__secret}`,
                        );
                    return;
                }
            ...
            client.on('puback', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    !adapter.config.ignorePings &&
                        adapter.log.warn(
                            `Old client ${client.id} with secret ${client.__secret} sends puback. Ignore! Actual secret is ${clients[client.id].__secret}`,
                        );
                    return;
                }
            ...
            client.on('unsubscribe', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    !adapter.config.ignorePings &&
                        adapter.log.warn(
                            `Old client ${client.id} with secret ${client.__secret} sends unsubscribe. Ignore! Actual secret is ${clients[client.id].__secret}`,
                        );
                    return;
                }
            ...
            client.on('subscribe', packet => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    !adapter.config.ignorePings &&
                        adapter.log.warn(
                            `Old client ${client.id} with secret ${client.__secret} sends subscribe. Ignore! Actual secret is ${clients[client.id].__secret}`,
                        );
                    return;
                }
            ...
            client.on('pingreq', (/*packet*/) => {
                if (clients[client.id] && client.__secret !== clients[client.id].__secret) {
                    !adapter.config.ignorePings &&
                        adapter.log.warn(
                            `Old client ${client.id} with secret ${client.__secret} sends pingreq. Ignore! Actual secret is ${clients[client.id].__secret}`,
                        );
                    return;
                }
        ...
        server.on('error', err => adapter.log.error(`Can not start Server ${err}`));
        ...
        config.storeClientsTime =
            config.storeClientsTime === undefined ? 1440 : parseInt(config.storeClientsTime, 10) || 0;
