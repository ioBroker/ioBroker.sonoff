    ...
    function receivedTopic(packet, client) {
        ...
        if (packet.topic.match(/^(stat|cmnd)\/.+\/POWER\d?$/)) {
            ...
            try {
                ...
                if (data.DS18x20 && data.DS18x20.DS1 && data.DS18x20.DS1.Temperature !== undefined) {
                    id = adapter.namespace + '.' + client.id + '.DS18x20_Temperature';
                    tasks.push({
                        type: 'addObject',
                        id: id,
                        data: {
                            _id: id,
                            common: {
                                name: client.id + ' DS18x20 Temperature',
                                type: 'number',
                                unit: data.TempUnit ? (data.TempUnit.indexOf('°') !== -1 ? '°' + data.TempUnit : data.TempUnit) : '°C',
                                role: 'value.temperature',
                                read: true,
                    ...
                    adapter.setState(id, parseFloat(data.DS18x20.DS1.Temperature), true);
                }
        ...
        } else if (packet.topic.match(/^tele\/.+\/ENERGY\d?$/)) {
            // {"Time":"2017-10-02T19:24:32", "Total":1.753, "Yesterday":0.308, "Today":0.205, "Period":0, "Power":3, "Factor":0.12, "Voltage":221, "Current":0.097}
            try {
                let data = JSON.parse(val);
                if (data.Total !== undefined) {
                    id = adapter.namespace + '.' + client.id + '.ENERGY.total';
                    tasks.push({
                        type: 'addObject',
                        id: id,
                        data: {
                    ...
                    adapter.setState(id, parseFloat(data.Total), true);
                }
                if (data.Yesterday !== undefined) {
                    id = adapter.namespace + '.' + client.id + '.ENERGY.yesterday';
                    tasks.push({
                        type: 'addObject',
                        id: id,
                        data: {
                            _id: id,
                            common: {
                                name: client.id + ' ENERGY Yesterday',
                                type: 'number',
                                unit: 'W/h',
                                role: 'value.power.consumption',
                                read: true,
                    ...
                    adapter.setState(id, parseFloat(data.Yesterday), true);
                }
                if (data.Factor !== undefined) {
                    id = adapter.namespace + '.' + client.id + '.ENERGY.factor';
                    tasks.push({
                        type: 'addObject',
                        id: id,
                        data: {
                    ...
                    adapter.setState(id, parseFloat(data.Factor), true);
                }
                if (data.Power !== undefined) {
                    id = adapter.namespace + '.' + client.id + '.ENERGY.power';
                    tasks.push({
                        type: 'addObject',
                        id: id,
                        data: {
                    ...
                    adapter.setState(id, parseFloat(data.Power), true);
                }
                if (data.Voltage !== undefined) {
                    id = adapter.namespace + '.' + client.id + '.ENERGY.voltage';
                    tasks.push({
                        type: 'addObject',
                        id: id,
                        data: {
                    ...
                    adapter.setState(id, parseFloat(data.Voltage), true);
                }
                if (data.Current !== undefined) {
                    id = adapter.namespace + '.' + client.id + '.ENERGY.current';
                    tasks.push({
                        type: 'addObject',
                        id: id,
                        data: {
                    ...
                    adapter.setState(id, parseFloat(data.Current), true);
                }
            } catch (e) {
                adapter.log.warn('Cannot parse data: ' + val);
            }
