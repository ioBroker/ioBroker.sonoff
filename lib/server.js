...
*/
    ...
    function addObject(attr, client, prefix, path) {
        ...
        adapter.log.warn('0. replaceAttr: "' + replaceAttr + '"');
        adapter.log.warn('1. addObject id: "' + id + '"');
        let obj = {
            type: 'addObject',
            id: id,
            data: {
        ...
        adapter.log.warn('2. addObject name: "' + obj.data.common.name + '"');
        return obj;
    }
            ...
            if (typeof data[attr] === 'object') {
                ...
                if (types[attr]) {
                    if (types[attr].type === 'array') {
                        ...
                        adapter.log.warn('is array"' + adapter.namespace + '": _' + client.iobId);
                        ...
                        adapter.log.warn('is no array1 "' + adapter.namespace + '":"' + client.iobId + '"');
                        adapter.log.warn('checkData1: "' + client + '":"' + topic + '":"' + prefix + '":"' + nPath + '"');
                    ...
                    let nPath = Object.assign([], path);
                    nPath.push(attr.replace(FORBIDDEN_CHARS, '_'));
                    adapter.log.warn('is no array2 "' + adapter.namespace + '":"' + client.iobId + '"');
                    adapter.log.warn('checkData2: "' + topic + '":"' + prefix + '":"' + data[attr] + '":"' + unit + '":"' + nPath + '"');
                    if (prefix.match(/SENSOR\d?/)) {
                        adapter.log.warn('prefix1: "' + prefix + '"');
                        prefix = 'SENSOR';
                        adapter.log.warn('prefix2: "' + prefix + '"');
                    }
                    //(prefix ? prefix + '.' : '')
                    prefix = prefix + '.' + nPath;
                    adapter.log.warn('prefix3: "' + prefix + '"');
                    // checkData(client, topic, prefix, data[attr], unit, nPath);
                    checkData(client, topic, prefix, data[attr], unit);
                    nPath = undefined;
                }
            } else if (types[attr]) {
                let allowReadColors;
				...
				if ((parts[0] === 'tele' && (
					...
					adapter.log.info('[' + client.id + '] My auto insert object: ' + attr + ' ' + data[attr]);
					let attributes;
					if (parts[2] === 'RESULT') {
						if (xdata.isNaN) {
							// string
        ...
        */
        ...
        if (parts[0] === 'tele' && stateId.match(/^STATE\d?$/)) {
            try {
                ...
                adapter.log.warn('log sensor parse: "' + stateId + '":"' + val + '"');
            } catch (e) {
                adapter.log.warn('Cannot parse data "' + stateId + '": _' + val + '_ - ' + e);
            }
