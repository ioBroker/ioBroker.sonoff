...
*/	
    ...
    function onStateChangedColors(id, state, channelId, stateId) {
        ...
        if (cachedModeExor[ledModeIdExor] === undefined) {
            return adapter.getForeignState(ledModeIdExor, (err, state) => {
                cachedModeExor[ledModeIdExor] = state ? state.val || false : true;
    ...
    function checkData(client, topic, prefix, data, unit, path) {
        ...
        const ledModeReadColorsID = adapter.namespace + '.' + client.id + '.modeReadColors';
            ...
            if (typeof data[attr] === 'object') {
                ...
                if (attr === 'Color') {
                    ...
                    if (hueCalc) {
                        ...
                        xObj.data.common.def = true;
                        updateState(xObj);

                        xObj = addObject('modeLedExor', client, prefix, path);
                        xObj.data.common.def = false;
                        updateState(xObj);
        ...
        if (adapter.config.storeClientsTime !== -1) {
            for (const id in persistentSessions) {
                if (persistentSessions.hasOwnProperty(id)) {
                    if (now - persistentSessions[id].ts > adapter.config.storeClientsTime * 60000) {
