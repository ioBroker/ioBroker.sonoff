...
*/
    ...
    function checkData(client, topic, prefix, data, unit, path) {
        ...
        // Extract pressure unit if available at this level for nested pressure data
        const pressureUnit = data.PressureUnit || unit?.pressureUnit;
            ...
            if (typeof data[attr] === 'object') {
                ...
                if (types[attr]) {
                    if (types[attr].type === 'array') {
                        ...
                        const nestedUnit = pressureUnit ? { pressureUnit } : unit;
                        checkData(client, topic, prefix, data[attr], nestedUnit, nPath);
                        nPath = undefined;
                    }
                } else {
                    let nPath = Object.assign([], path);
                    ...
                    if (adapter.config.OBJ_TREE) {
                        checkData(client, topic, `${prefix}.${nPath}`, data[attr], nestedUnit);
                    } else {
                        checkData(client, topic, prefix, data[attr], nestedUnit, nPath);
                    }
                    nPath = undefined;
