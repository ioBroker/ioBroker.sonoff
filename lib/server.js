...
*/
    ...
    async function ensureDeviceObject(client) {
    try {
        if (!client || !client.iobId) {
            return;
        }

        const deviceId = `${adapter.namespace}.${client.iobId}`;

        try {
            const obj = await adapter.getObjectAsync(deviceId);
            if (obj && obj.type === 'device') {
                obj.common = obj.common || {};
                obj.common.statusStates = obj.common.statusStates || {};
                if (!obj.common.statusStates.onlineId) {
                    obj.common.statusStates.onlineId = `${deviceId}.alive`;
                    await adapter.setObjectAsync(deviceId, obj);
                }
                return;
            }
        } catch (err) {
        ...
        await adapter.setObjectAsync(deviceId, {
            _id: deviceId,
            type: 'device',
            common: {
                name: client.id,
                statusStates: {
                    onlineId: `${deviceId}.alive`,
                },
            },
            native: {
                clientId: client.id,
        ...
        adapter.log.warn(`Could not ensure device object for "${client.id}": ${err.message}`);
    }
}

    async function createClient(client, callback) {
    try {
        // mqtt.0.cmnd.sonoff.POWER
        // mqtt.0.stat.sonoff.POWER
        let isStart = !tasks.length;

        const baseId = `${adapter.namespace}.${client.iobId}`;

        // Device-Objekt für Admin-Statusfarben
        try {
            await ensureDeviceObject(client);
        } catch (e) {
            // Ignore error
        }

        // Channel unterhalb des Device
        let obj = {
            _id: baseId,
            common: {
                name: client.id,
        ...
        // Alive-State, der auch in statusStates.onlineId eingetragen ist
        obj = {
            _id: `${baseId}.alive`,
            common: {
                type: 'boolean',
        ...
        adapter.log.error(`Cannot create client: ${e}`);
    ...
    try {
        ...
        if (clients) {
            for (const id in clients) {
                try {
                    const oid = `${info.clients}.${id.replace(/\./g, '_').replace(FORBIDDENCHARS, '')}`;
                    clientIds.push(oid);
                    ...
                    if (!clientObj?.native) {
                        ...
                        adapter.log.debug(`Created client state for ${id}`);
                    } else if (
                        clients[id] &&
                        ...
                        // ✨ KRITISCHER FIX: await + Async statt setObject
                        await adapter.setObjectAsync(clientObj._id, clientObj);
                        adapter.log.debug(`Updated client state for ${id} - IP: ${clientObj.native.ip}, Port: ${clientObj.native.port}`);
                    }
                } catch (err) {
                    adapter.log.warn(`Error processing client ${id}: ${err.message}`);
                    continue;
                }
            }
        ...
        try {
            ...
            adapter.log.warn(`Error setting offline states: ${err.message}`);
        }
        ...
        try {
            ...
            await adapter.setStateAsync(`${info.connection}`, text, true);
            adapter.log.debug(`Updated connection state: ${text || 'disconnected'}`);
        } catch (err) {
            adapter.log.warn(`Error updating connection state: ${err.message}`);
        }
        
    } catch (err) {
        adapter.log.error(`Critical error in updateClients: ${err.message}`);
