...
function MQTTServer(adapter) {
	...
	*/
    ...
    this.onStateChange = (id, state) => {
        ...
        if (server && state && !state.ack) {
            ...
            if (clients[channelId]) {
				...
				var hid_e=id.split(".");
				var device_desc=hid_e[0] + '.' + hid_e[1] + '.' + hid_e[2];
                if (stateId.match(/Red\d?/)) {
					// set red component
					if (state.val>100) state.val=100;
					var red=toPaddedHexString(Math.floor(255*state.val/100),2);
					var idAlive=device_desc + '.Color';
	                adapter.getForeignState(idAlive, function (err, state) {
	                    if (!state) {
							adapter.setState(idAlive,'#000000', false);
	                        return;
	                    }
						var color=state.val.substring(1);
						// replace red component
						var out=red + color.substring(2,10);
                ...
                if (stateId.match(/Green\d?/)) {
					// set green component
					if (state.val>100) state.val=100;
					var green=toPaddedHexString(Math.floor(255*state.val/100),2);
					var idAlive=device_desc + '.Color';
	                adapter.getForeignState(idAlive, function (err, state) {
	                    if (!state) {
							adapter.setState(idAlive,'#000000', false);
						...
						var out=color.substring(0,2) + green + color.substring(4,10);
						//adapter.setState(idAlive,'#' + out, false);
						SetColor(channelId,out);
					});
					return;
                ...
                if (stateId.match(/Blue\d?/)) {
					// set blue component
					if (state.val>100) state.val=100;
					var blue=toPaddedHexString(Math.floor(255*state.val/100),2);
					var idAlive=device_desc + '.Color';
	                adapter.getForeignState(idAlive, function (err, state) {
	                    if (!state) {
							adapter.setState(idAlive,'#000000', false);
						...
						var out=color.substring(0,4) + blue + color.substring(6,10);
						//adapter.setState(idAlive,'#' + out, false);
						SetColor(channelId,out);
					});
					return;
				...
				if (hue_calc) {
					...
					if (stateId.match(/Hue\d?/)) {
						var hue=state.val;
						if (hue>359) hue=359;
						// recalc color by hue
						var idAlive=device_desc + '.Dimmer';
				        adapter.getForeignState(idAlive, function (err, state) {
							var dim;
	                    	if (!state) {
								dim=100;
							...
							var dim=state.val;
							let idAlive=device_desc + '.Saturation';
			                adapter.getForeignState(idAlive, function (err, state) {
								var sat;
			                    if (!state) {
									sat=100;
								...
								var hexval=componentToHex(rgb[0]) + componentToHex(rgb[1]) + componentToHex(rgb[2]);
								let idAlive=device_desc + '.Color';
					...
					if (stateId.match(/Saturation\d?/)) {
						var sat=state.val;
						if (sat>100) sat=100;
						// recalc color by saturation
						let idAlive=device_desc + '.Dimmer';
				        adapter.getForeignState(idAlive, function (err, state) {
							var dim;
	                    	if (!state) {
								dim=100;
                	...
                	if (!obj) {
			            ...
			            if (stateId.match(/CW_POWER\d?/)) {
				            ...
				            adapter.getForeignState(idAlive, function (err, state) {
								...
								var out=color.substring(0,6) + cw + color.substring(8,10);
								SetColor(channelId,out);
								// in case POWER is off, switch it on
								if (cwpow==true) {
									let idAlive=device_desc + '.POWER';
						...
						// set ww component
							var ww=toPaddedHexString(Math.floor(255*state.val/100),2);
							var idAlive=device_desc + '.Color';
	                		adapter.getForeignState(idAlive, function (err, state) {
	                    		if (!state) {
									adapter.setState(idAlive,'#000000', false);
						...
						// set ww component
							var cw=toPaddedHexString(Math.floor(255*state.val/100),2);
							var idAlive=device_desc + '.Color';
	                		adapter.getForeignState(idAlive, function (err, state) {
	                    		if (!state) {
									adapter.setState(idAlive,'#000000', false);
    ...
    const types = {
		...
		BITS:          {type: 'number',  role: 'value',                    read: true, write: true},
        ...
        for (let attr in data) {
            ...
            if (types[attr]) {
				...
				if (attr === 'Color') {
					...
					if (hue_calc) {
						var xobj=AddObject('Hue',client,prefix,path);
						tasks.push(xobj[0]);
						...
						tasks.push(xobj[0]);
						var val=parseInt(data[attr].substring(0,6), 16)
						if (val>0) {
    ...
    function receivedTopic(packet, client) {
        ...
        if (stateId === 'RESULT') {
			...
			if (packet.payload.indexOf("POWER")>=0) return;
	        if (parts[0] === 'stat') {
	            try {
