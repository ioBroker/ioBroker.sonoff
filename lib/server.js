...
// 1=tele/sonoff/SENSOR
// 2=tele/sonoff/STATE
// 4=stat/sonoff/RESULT
// overwritten by setup/flags
var auto_flags = 1;
//var heap_test = 0;
...
<<<<<<< HEAD
				...
				const locvar = specvars.indexOf(stateId); 
				if (locvar<0) {
					// other objects
...
=======
                if (stateId.startsWith('POWER')) {
                    if (clients[channelId]._map && clients[channelId]._map[stateId]) {
                        setImmediate(sendState2Client, clients[channelId], clients[channelId]._map[stateId], state.val ? 'ON' : 'OFF');
                    } else if (clients[channelId]._fallBackName) {
                ...
                if (stateId.match(/Dimmer\d?/)) {
                    if (clients[channelId]._map && clients[channelId]._map[stateId]) {
                        setImmediate(sendState2Client, clients[channelId], clients[channelId]._map[stateId], state.val.toString());
                    } else if (clients[channelId]._fallBackName) {
                ...
                if (stateId.match(/Wakeup\d?/)) {
                    if (clients[channelId]._map && clients[channelId]._map[stateId]) {
                        setImmediate(sendState2Client, clients[channelId], clients[channelId]._map[stateId], state.val);
                    } else if (clients[channelId]._fallBackName) {
                        setImmediate(sendState2Client, clients[channelId], 'cmnd/' + clients[channelId]._fallBackName + '/' + stateId, state.val.toString());
                    } else {
                        adapter.log.warn('Unknown mapping for "' + stateId + '"');
...
=======
                            ...
                            if (clients[channelId]._map && clients[channelId]._map[stateId]) {
                                setImmediate(sendState2Client, clients[channelId], clients[channelId]._map[stateId], color);
                            } else if (clients[channelId]._fallBackName) {
...
>>>>>>> ioBroker/master
    ...
    function checkData(client, topic, prefix, data, unit, path) {
        ...
        for (let attr in data) {
            ...
            if (typeof data[attr] === 'object') {
				...
				//adapter.log.info('obj: ' + attr + data[attr]);

				// create object
				const cobj=addObject(attr, client, prefix, path)
				let obj=cobj[0];
				let id=cobj[1];
				...
				const id=adapter.namespace + '.setup.flags'
                adapter.getForeignObject(id, function (err, obj) {
                    if (!obj) {
            			...
            			let obj = {
                			...
                			data: {
                    			_id: id,
                    			common: Object.assign({},{type:'number',role:'value',read:true,write:true}),
                    			native: {},
                    			type: 'state'
                			}
            			};
						obj.data.common.name = 'flags'
	            		adapter.setForeignObject(obj.id,obj.data, (/* err */) => {
							adapter.log.info("new object created: " + id);
	            		});
						adapter.setState(id,1, true);
                    } else {
	                	adapter.getForeignState(id, function (err, state) {
	                        if (!state) {
	                            adapter.log.warn('ill flags state');
							} else {
								//adapter.log.info('flags: ' + state.val);
								auto_flags=state.val;
							}
						});
					}
				});
				...
				if (auto_flags) {
					//adapter.log.info('[' + client.id + '] auto insert object: ' + attr + ' ' + data[attr] + ' flags: ' + auto_flags);
					let parts = topic.split('/');
					// auto generate objects
					if ( (parts[0]==='tele' && ((auto_flags&1 && parts[2]==='SENSOR') || (auto_flags&2 && parts[2]==='STATE'))) || (parts[0]==='stat' && (auto_flags&4 && parts[2]==='RESULT')) ) {
						//adapter.log.info('[' + client.id + '] auto insert object: ' + attr + ' ' + data[attr] + ' flags: ' + auto_flags);
						//if (data[attr]) {
							var xdata=data[attr];
                			...
                			obj.data.common.name = client.id + ' ' + (prefix ? prefix + ' ' : '') + (path.length ? path.join(' ') + ' ' : '') + ' ' + replaceAttr;
							pushObject(id,obj);
							adapter.setState(id,xdata, true);
							obj=undefined;
							id=undefined;
    ...
    function receivedTopic(packet, client) {
		...
		/*
		adapter.getForeignState('system.adapter.sonoff.0.memRss', function (err, state) {
			adapter.log.info('mem: ' + state.val + ' MB');
			if (heap_test==0 || (heap_test==1 && state.val>37.5)) {
				heap_test+=1;
