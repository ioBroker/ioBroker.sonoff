...
export interface MQTTPacketStored extends MQTTPacket {
    ts: number;
    cmd: 'pubrel' | 'publish';
    count: number;
}
...
export default abstract class MQTTBase {
    protected readonly clients: { [key: string]: MQTTClient } = {};
    protected readonly tasks: Task[] = [];
    protected taskCallbacks: (() => void)[] = [];
    protected mappingClients: { [iobId: string]: string } = {};
    protected readonly adapter: ioBroker.Adapter;

    protected readonly cacheAddedObjects: { [id: string]: boolean } = {};
    protected readonly cachedModeExor: { [id: string]: boolean } = {};
    protected readonly cachedReadColors: { [id: string]: boolean } = {};
    protected readonly cachePowerObjects: { [id: string]: true } = {};
    protected readonly specVars = [
        'Red',
        'Green',
        'Blue',
        'WW',
    ...
    protected readonly config: SonoffAdapterConfig;

    constructor(adapter: ioBroker.Adapter) {
        this.adapter = adapter;
        this.config = adapter.config as SonoffAdapterConfig;
        this.config.defaultQoS = (parseInt(this.config.defaultQoS as unknown as string, 10) as 0 | 1 | 2) || 0;
    }

    protected abstract sendState2Client(
        client: MQTTClient,
        topic: string,
    ...
    protected updateState(task: Task, val?: ioBroker.StateValue | (() => void), callback?: () => void): void {
     ...
     */
    private getChannelId(id: string): string {
    ...
    protected createClient(client: MQTTClient, callback: () => void): void {
    ...
    private checkData(
        ...
        for (const attr in data) {
            ...
            if (typeof data[attr] === 'object') {
                ...
                let val: number | string | boolean | undefined;
                ...
                if (obj.id.includes('.STATE.POWER')) {
                    ...
                    this.syncPowerState(powerId, val ?? null).catch(err =>
    ...
    protected async receivedTopic(packet: MQTTPacket, client: MQTTClient): Promise<void> {
        ...
        if (parts[0] === 'tele' && stateId.match(/^(RESULT|STATE|SENSOR|WAKEUP)\d?$/)) {
            ...
            try {
                const data = JSON.parse(val) as { StatusNET?: Record<string, unknown> };
            ...
            try {
                const data = JSON.parse(val) as { StatusFWR?: Record<string, unknown> };
